...
class SingleRegionModel_fixedFirstStage:
    
    def __init__(self, short_term_scenarios, num_short_term_scenarios ,first_stage_solution):
        self.model = ConcreteModel()
        self.short_term_scenarios = short_term_scenarios
        self.num_short_term_scenarios = num_short_term_scenarios
        self.first_stage_solution = first_stage_solution
        self.build_model()
    ...
    def build_model(self):
        ...
        self.model.N = Set(initialize=range(self.num_short_term_scenarios))
        self.model.T = Set(initialize=range(int(scenarios_infos['scenario_length'].iloc[0]) * self.num_short_term_scenarios))
        ...
        prob = 1/self.num_short_term_scenarios
        ...
        self.model.mu_e = Param(self.model.I, initialize = {index:float(co2_budget_scaling['value'].iloc[index+1] * other_params['co2_lim (ktCO2)'].iloc[0]) for index, value in enumerate(self.model.I)})
        ...
        # investment planning variable sbecome fixed so parameters
        self.model.x_acc = Param(self.model.P, self.model.I, initialize = self.first_stage_solution[0])
        self.model.x_inst = Param(self.model.P, self.model.I_0, initialize = self.first_stage_solution[1])
